 ...
 * 
 */
public class VerifyEmailActivity extends BaseActivity implements OnClickListener {
	...
	private void initView() {
		...
		if (status < 'd') {
			...
			emailEditText.setTextColor(getResources().getColor(R.color.gray_333333));
			...
			emailEditText.setTextColor(getResources().getColor(R.color.gray));
	...
	private boolean checkValue() {
		...
		if (TextUtils.isEmpty(emailStr)) {
			...
			ActivityUtil.shakeView(emailEditText);
			return false;
	...
	// 查询数据
	private void requestEmailValue() {
		HashMap<String, String> tempMap = new HashMap<String, String>();
		tempMap.put("type", "EMAIL");
			...
			public void onResponse(String jsonObject) {
				...
				objectMapper.configure(DeserializationConfig.Feature.FAIL_ON_UNKNOWN_PROPERTIES,false);
					...
					if (dto.getStatus() == AppResponseStatus.SUCCESS) {
						...
						if (status < 'd') {
							emailEditText.setText(map.get("EMAIL"));
						...
						Toast.makeText(VerifyEmailActivity.this, dto.getMsg(), Toast.LENGTH_SHORT).show();
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void requestSaveWechat() {
		HashMap<String, String> tempMap = new HashMap<String, String>();
		tempMap.put("type", "EMAIL");
		tempMap.put("value", emailEditText.getText().toString().trim());
